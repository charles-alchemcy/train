...
def process_shard(
    ...
    # Build records - convert to list only for kept items
    kept = []
    for i in kept_indices[:args.keep_per_shard]:
        kept.append({
...
def main() -> None:
    ...
    try:
        ...
        files = [f for f in files if args.shard_start <= int(f.split("_")[1].split(".")[0]) < args.shard_end]
        rng.shuffle(files)
        ...
        with output_path.open("w") as output_file:
            for file in files:
                ...
                # Progress logging - now using stats dict
