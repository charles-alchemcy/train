...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        default="percentile",
        help="Selection strategy: 'percentile' (range by percentile) or 'threshold' (loss >= value)"
    )
    parser.add_argument(
        "--min-loss-percentile", 
        type=float, 
        default=35.0,
        help="Min percentile for filtering (only used in percentile mode)"
    )
    parser.add_argument(
        "--max-loss-percentile", 
        type=float, 
        default=98.0,
        help="Max percentile for filtering (only used in percentile mode)"
    )
    parser.add_argument(
        "--loss-threshold", 
        type=float, 
        default=None,
        help="Loss threshold for filtering (only used in threshold mode; select samples with loss >= threshold)"
    )
...
def process_shard(
    ...
    if args.selection_mode == "percentile":
        ...
        if args.loss_threshold is None:
            raise ValueError("--loss-threshold must be set when using --selection-mode threshold")
        threshold = args.loss_threshold
        keep_mask = candidate_losses >= threshold
        stats["lo"], stats["hi"] = threshold, float("inf")
    ...
    # Shuffle to randomize output order (no truncation/sorting)
    if kept:
        rng.shuffle(kept)
...
def main() -> None:
    ...
    # Validate arguments
    if args.selection_mode == "threshold" and args.loss_threshold is None:
        raise ValueError("--loss-threshold must be specified when using --selection-mode threshold")
    ...
    try:
        ...
        with output_path.open("w") as output_file:
            for file in filtered_files:
                ...
                # Format band display based on mode
                if args.selection_mode == "percentile":
                    band_str = f"[{stats['lo']:.2f}, {stats['hi']:.2f}]"
                else:
                    band_str = f">={stats['lo']:.2f}"
                
                print(
                    f"Shard {shard_idx:4d}: cand={stats['candidate_n']:4d} "
                    f"avg_loss={stats['avg_loss']:.4f} kept={stats['kept_count']:4d} "
                    f"kept_avg={stats['kept_avg']:.4f} band={band_str} "
