...
class RotaryEmbedding(nn.Module):
    def __init__(self, dim, max_position_embeddings=4096, base=100000, device=None):
        ...
        t = torch.arange(
            max_position_embeddings + 1, device=device, dtype=inv_freq.dtype
        )
    ...
    def forward(self, x, seq_len=None):
        if seq_len is not None and seq_len > self._cos_cached.shape[2]:
            ...
            self.register_buffer(
                "_cos_cached",
                emb.cos()[None, None, :, :].to(self._cos_cached.dtype),
                persistent=False,
            )
            self.register_buffer(
                "_sin_cached",
                emb.sin()[None, None, :, :].to(self._sin_cached.dtype),
                persistent=False,
            )
        return (
...
class LatentMemoryModule(nn.Module):
    ...
    def write_memory(self, H, M, chunk_idx=0):
        ...
        scores = (
            torch.bmm(Z_seg, M.transpose(-1, -2))
            * self.scale
...
class BigMacMoE(nn.Module):
    ...
    def __init__(self, config, layer_idx=None):
        ...
        default_routed_size = int(
            getattr(config, "routed_expert_size", 768) / self.bigmac_r
        )
        self.routed_expert_size = getattr(
            config, "bigmac_expert_size", default_routed_size
        )
        ...
        self.experts_w12 = nn.Parameter(
            torch.zeros(
                self.num_routed_experts, self.bottle_dim, 2 * self.routed_expert_size
            )
        )
        self.experts_w3 = nn.Parameter(
            torch.zeros(
                self.num_routed_experts, self.routed_expert_size, self.bottle_dim
            )
        )
        ...
        self.register_buffer(
            "_dummy_token",
            torch.zeros(1, self.bottle_dim, dtype=torch.bfloat16),
            persistent=False,
        )
        self.register_buffer(
            "_dummy_out",
    ...
    def forward(self, x, expert_bias=None):
        ...
        with torch.autocast(device_type=x.device.type, dtype=torch.float32):
            ...
            bias = (
                ...
                else torch.zeros(self.num_routed_experts, device=x.device)
            )
        ...
        if self.training:
            ...
            expert_counts = torch.bincount(
                flat_topk_idx, minlength=self.num_routed_experts
            )
        ...
        used_counts = (
            ...
            else torch.bincount(flat_topk_idx, minlength=self.num_routed_experts)
        )
        ...
        valid_slots = (
            sorted_experts[capacity_mask] * max_load + local_idx[capacity_mask]
        )
        ...
        reverse_map.scatter_(
            0, valid_slots.long(), token_indices[permutation][capacity_mask]
        )

        padding = hidden_with_dummy.index_select(0, reverse_map).view(
            self.num_routed_experts, max_load, self.bottle_dim
        )
        ...
        gather_map = torch.full(
            (num_tokens_total,), num_slots, device=x.device, dtype=torch.long
        )
        gather_map.scatter_(0, permutation[capacity_mask], valid_slots)

        gathered_out = padded_out_with_dummy.index_select(0, gather_map).view(
            N, K, self.bottle_dim
        )

        routed_out_bottle = torch.bmm(
            gating_scores.to(gathered_out.dtype).unsqueeze(1), gathered_out
        ).squeeze(1)
        routed_out = self.w_up_proj(routed_out_bottle)

        if self.training:
            mean_load = num_tokens_total / self.num_routed_experts
            self._pending_violation = (mean_load - used_counts.float()) / (
                mean_load + 1e-6
            )
    ...
    def update_bias(self, counts, num_tokens):
        ...
        self.expert_momentum.data = (
            self.smebu_beta * self.expert_momentum.data
            + (1 - self.smebu_beta) * delta_bi
        )
        self.expert_bias.data = (
            (self.expert_bias.data + self.expert_momentum.data)
            .nan_to_num_()
            .clamp(-10.0, 10.0)
        )
...
class GroupedMoE(nn.Module):
    ...
    def __init__(self, config, layer_idx=None):
        ...
        self.experts_w12 = nn.Parameter(
            torch.zeros(
                self.num_routed_experts, self.d_model, 2 * self.routed_expert_size
            )
        )
        self.experts_w3 = nn.Parameter(
            torch.zeros(self.num_routed_experts, self.routed_expert_size, self.d_model)
        )
    ...
    def forward(self, x, expert_bias=None):
        ...
        if self.training:
            ...
            aux_loss = torch.tensor(0.0, device=x.device)
            expert_counts = None
            self._pending_violation = torch.zeros(
                self.num_routed_experts, device=x.device
            )
        ...
        padded_x = torch.zeros(
            num_experts, max_tokens, D, device=x.device, dtype=x.dtype
        )
        expert_starts = torch.cat(
            [torch.tensor([0], device=x.device), tokens_per_expert[:-1].cumsum(0)]
        )
        intra_offsets = torch.arange(
            N * K, device=x.device
        ) - expert_starts.repeat_interleave(tokens_per_expert)
    ...
    def _init_weights(self):
        ...
        def apply_deep_init(m):
            if hasattr(m, "down") and isinstance(m.down, nn.Linear):
                nn.init.normal_(
                    m.down.weight, mean=0.0, std=trinity_std * self.scale_factor
                )
        ...
        if hasattr(self.attn, "o_proj") and isinstance(self.attn.o_proj, nn.Linear):
            nn.init.normal_(
                self.attn.o_proj.weight, mean=0.0, std=trinity_std * self.scale_factor
            )
    ...
    def forward(
        ...
        **kwargs,
        ...
        return self._forward(
            x, cos, sin, expert_bias, memory_state, lambda_reg, **kwargs
        )
    ...
    def _forward(
        ...
        **kwargs,
        ...
        if self.layer_type == "gla" and memory_state is not None:
            new_memory_state, total_mem_loss, _ = self.memory.write_memory(
                x, memory_state
            )
...
class QuasarModel(QuasarPreTrainedModel):
    ...
    def __init__(self, config: QuasarConfig):
        ...
        self.moe_layer_ffns = [
            l.ffn for l in self.layers if getattr(l, "is_moe", False)
        ]
        ...
        if self.num_moe > 0 and num_experts > 0:
            ...
            self.register_buffer(
                "all_moe_momentum", torch.zeros(self.num_moe, num_experts)
            )
    ...
    def forward(
        ...
        **kwargs,
        ...
        # RoPE
        max_pos = (
            int(position_ids.max().item() + 1) if position_ids.numel() > 0 else seq_len
        )
        ...
        if memory_states is None:
            memory_states = self.init_memory(
        ...
        for loop_idx in range(num_loops):
            ...
            for layer in self.layers:
                ...
                bias = (
                    bias_snapshot[moe_idx]
                    if (getattr(layer, "is_moe", False) and bias_snapshot is not None)
                    else None
                )
...
class QuasarForCausalLM(QuasarPreTrainedModel, FLAGenerationMixin):
            ...
            if mask.any():
                ...
                for i in range(0, total_tokens, chunk_size):
                    ...
                    chunk_loss = F.cross_entropy(
                        chunk_logits.float(), active_labels[i:end], reduction="sum"
                    )
    ...
    def update_model_kwargs_for_generation(
        self, outputs, model_kwargs, is_seq2seq=False, num_new_tokens=1
    ):
